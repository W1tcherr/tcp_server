...
{
    server_fd = ErrorProc::Socket(domain, type, protocol); //(AF_INET, SOCK_STREAM, 0)
...
void Tcpserver::server_start(int backlog)
{
    ...
    ErrorProc::Bind(server_fd, (struct sockaddr*) &server_address, server_address_len);
    ErrorProc::Listen(server_fd, backlog); //backlog = 10
    client_fd = ErrorProc::Accept(server_fd, (struct sockaddr*) &server_address, &(server_address_len));
